...
The URL which will be called with the event payload on configured operation. Must be a `POST` handler. This URL
...
Hasura supports two URL schemes:

- **`https://`** - Standard HTTP webhook endpoint
- **`aws://`** - Direct AWS Lambda invocation using the function ARN

**Examples:**

```
https://my-webhook.example.com/handler
...
For `aws://` URLs, Hasura automatically signs requests using AWS Signature V4. Set the following environment variables:

- `AWS_ACCESS_KEY_ID`
- `AWS_SECRET_ACCESS_KEY`
- `AWS_SESSION_TOKEN` (optional, for temporary credentials)

The optional `?invocation_type=event` query parameter enables asynchronous Lambda invocation.
