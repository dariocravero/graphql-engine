...
You can configure Hasura to redact sensitive field values from logs using the `--redacted-log-fields` flag or the
`HASURA_GRAPHQL_REDACTED_LOG_FIELDS` environment variable. This is useful for preventing sensitive data like passwords,
tokens, and API keys from appearing in logs.

### Configuration

The value should be a JSON array of field names to redact:

```bash
HASURA_GRAPHQL_REDACTED_LOG_FIELDS='["password", "token", "secret", "api_key", "authorization"]'
```

Or using the command-line flag:

```bash
graphql-engine serve --redacted-log-fields='["password", "token", "secret"]'
```

### Field matching

Field matching is **case-insensitive** and **ignores underscores and hyphens**. This means:

| Configured field | Will also match |
| --------------- | --------------- |
| `password` | `Password`, `PASSWORD`, `pass_word`, `PASS-WORD` |
| `apiKey` | `api_key`, `API_KEY`, `api-key`, `ApiKey` |
| `accessToken` | `access_token`, `ACCESS_TOKEN`, `access-token` |

### Example

With `HASURA_GRAPHQL_REDACTED_LOG_FIELDS='["password", "token"]'`, a query log like this:
...
Log redaction is applied at the logging layer before any log is written. When no fields are configured for redaction,
there is zero performance overhead. When redaction is enabled, the overhead is minimal (typically < 0.5ms per request)
as the implementation uses pre-compiled field matching with O(1) hash lookups.
