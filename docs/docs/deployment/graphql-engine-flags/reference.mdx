...
### Redacted Log Fields {#redacted-log-fields}

A JSON array of field names whose values should be redacted from all log output. This is useful for preventing
sensitive data like passwords, tokens, and API keys from appearing in logs. See
[Log Redaction](/deployment/logging.mdx#log-redaction) for more details.

Field matching is case-insensitive and ignores underscores/hyphens (e.g., `password` matches `Password`, `pass_word`,
`PASS-WORD`).

|                     |                                                                                     |
| ------------------- | ----------------------------------------------------------------------------------- |
| **Flag**            | `--redacted-log-fields <JSON_ARRAY>`                                                |
| **Env var**         | `HASURA_GRAPHQL_REDACTED_LOG_FIELDS`                                                |
| **Accepted values** | JSON array of strings                                                               |
| **Example**         | `["password", "token", "secret", "api_key"]`                                        |
